  ...
  .then(function (data) {
    ...
    if (data.data.length >= 1) {
      ...
      <div class="tripSelect " style="display: visible;">
        ...
        }</span> <span class='price'>${data.data[i].price}€</span>
      ...
      for (let i = 0; i < document.querySelectorAll(".btnSupp").length; i++) {
        document
          .querySelectorAll(".btnSupp")
          [i].addEventListener("click", function () {
            fetch(
              `http://localhost:3000/carts/deletecart?departure=${data.data[i].departure}&arrival=${data.data[i].arrival}&price=${data.data[i].price}&date=${data.data[i].date}`,
              { method: "DELETE" }
            )
              .then((response) => response.json())
              .then((data) => {
                ...
                for (
                  let i = 0;
                  i < document.querySelectorAll(".price").length;
                  i++
                ) {
                  numbers.push(
                    Number(
                      document
                        ...
                        [i].textContent.slice(
                          document.querySelectorAll(".price")[i].length,
                          -1
                        )
                    )
                  );
...
document.querySelector("#buyTrip").addEventListener("click", () => {
  fetch(`http://localhost:3000/carts`)
    ...
    .then((data) => {
      ...
      for (let i = 0; i < data.data.length; i++) {
        fetch(
          `http://localhost:3000/bookings/book?departure=${data.data[i].departure}&arrival=${data.data[i].arrival}&date=${data.data[i].date}&price=${data.data[i].price}`,
          {
            method: "POST",
