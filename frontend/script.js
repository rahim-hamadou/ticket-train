...
btnSearch.addEventListener("click", () => {
	...
	fetch(
		`https://train-backend-demo.vercel.app/trips?departure=${departureInput}&arrival=${arrivalInput}&date=${dateInput}`,
	)
		...
		.then((data) => {
			if (data.data.length === 0) {
  			...
  			<span class='time-result'>${hours}:${minutes < 10 ? "0" + minutes : minutes}</span>
  			...
  			`;
					for (let i = 0; i < document.querySelectorAll(".bookBtn").length; i++) {
						document.querySelectorAll(".bookBtn")[i].addEventListener("click", () => {
							fetch(
								`https://train-backend-demo.vercel.app/carts/cart?departure=${data.data[i].departure}&arrival=${data.data[i].arrival}&date=${data.data[i].date}&price=${data.data[i].price}`,
								{
									method: "POST",
